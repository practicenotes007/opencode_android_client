...
## 2026-06-09：双用户 opencode serve 内存容量评估

### 背景

为支持多客户，在服务器上启动了 2 个 opencode serve 实例，通过 nginx Basic Auth → `$remote_user` map 路由：
- `devinoc` → `127.0.0.1:3080`（`opencode-serve.service`）
- `wxp` → `127.0.0.1:3081`（`opencode-serve-wxp.service`）

两个 serve 共享 `/home/ubuntu/.local/share/opencode/opencode.db`（SQLite）。

### 问题

devinoce serve 在运行 3 天后累积到 **1.1GB 内存峰值**（systemd `Memory peak` 统计），加上 wxp serve（peak 466MB）和交互式 CLI（~747MB），三进程峰值合计 **~2.3GB**，超过服务器 **3.8GB 物理内存 + 4GB swap** 的可用容量。内核在 6 月 6 日触发 OOM kill，杀掉 devinoc serve。

### 实测数据

| 进程 | 轻载 | 峰值 | 3 天累积 |
|------|------|------|----------|
| devinoc serve | 329MB | 461MB | **1.1GB** |
| wxp serve | 309MB | 466MB | 325MB |
| 交互式 CLI | ~700MB | ~750MB | — |

### 结论：3.8GB 无法可靠承载双用户

- server 内存在长运行中会持续增长（SQLite WAL 未 checkpoint、session context 积累等）
- 任意一个 serve 处理重负载（大 session、多 sub-agent 并发）即可触发 OOM
- 升级到 8GB+ 或分离到两台机器才能保证生产级稳定性
- 临时的缓解手段：定期重启 serve（cron/systemd timer 每天一次），将内存峰值重置回 ~400MB
